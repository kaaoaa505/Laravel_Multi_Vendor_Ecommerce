@extends('_backend._layout._main')
@section('content')
    @php
        $date = date('d-m-y');
        $today = null;//App\Models\Order::where('order_date', $date)->sum('amount');

        $month = date('F');
        $month = null;//App\Models\Order::where('order_month', $month)->sum('amount');

        $year = date('Y');
        $year = null;//App\Models\Order::where('order_year', $year)->sum('amount');
